...
- **Outbound HTTPS to AMD KDS** (`kdsintf.amd.com`), which the verifier uses to fetch the VCEK
  when verifying a bare SEV-SNP report. (A discovery document ships the VCEK inline, so the LB
  path verifies offline. TDX quotes carry their certification chain inline and verify against
  the verifier's embedded Intel root — no Intel endpoint is needed.)
...
`c8s cds verify` is a shorthand for `c8s verify --kind cds --mode auto` (default port
`8443`). `mode` auto-resolves to `ratls-cert` from `kind`.
`8443`) `. `mode` auto-resolves to `ratls-cert` from `kind`. The CDS serves [RA-TLS](/docs/c8s/attestation/cds#verifying-the-cds-itself-ra-tls): its
...
[image allowlist](/docs/c8s/attestation/allowlist#authorizing-mutations), fetched from
...
Compare them against the keys you pinned at install:

```bash
openssl pkey -pubin -in operator.pub -outform DER | sha256sum
```

As the output says, the key list is CDS-reported config, **not** part of the launch
...
disabled it reads `operator keys: endpoint reports no pinned operator keys (allowlist writes
disabled)`.
...
[`sev-snp-measure`](https://github.com/virtee/sev-snp-measure); Intel TDX is covered at the end
of the section.
...
the image isn't what you think it is.
...
pin the predicted value exactly as above. The runtime measurement registers (`RTMR`s), which

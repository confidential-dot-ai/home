---
title: CLI Reference
description: Every flag of the c8s CLI — install, uninstall, render-values, operator, cds, cds-attest, verify, get-cert, ratls-mesh, and nri-image-policy — with types, defaults, and descriptions.
---
...
| `--cvm-mode` | string | `baremetal` | No | CVM platform shape — selects the TEE device: `baremetal` or `gke` (native /dev/sev-guest) or `aks` (vTPM /dev/tpm0). All modes render a privileged attestation-api |
...
| `--cvm-mode` | string | `baremetal` | No | CVM platform shape selecting the TEE device: `baremetal` or `gke` (native /dev/sev-guest) or `aks` (vTPM /dev/tpm0) |
...
| `--allowlist-write-measurements` | string slice | `nil` | No | SHA-384 hex launch measurements allowed to mutate the allowlist via a bearer EAR |

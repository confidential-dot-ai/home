...
description: How the Certificate Distribution Service comes up — the in-memory mesh CA, token signing and JWKS rotation, the challenge/attest flow, leaf signing, CA handoff, and how clients verify the CDS itself.
---

<JourneyNav current={4} />

The **Certificate Distribution Service (CDS)** is the trust root of a c8s cluster. In one
process it verifies TEE attestation evidence, issues short-lived **attestation-bound tokens**, manages an in-memory
...
attestation-bound **bearer credential**: a caller presents the token to authorize an
[allowlist write](/docs/c8s/attestation/allowlist#authorizing-mutations) or a [CA handoff](#ca-handoff).
(Certificate issuance does **not** use this token — `/attest` verifies evidence and returns the signed
...
secret — a writer attests to the CDS (`/attest-key`), receives a short-lived
[CDS-signed token](#token-signing-and-jwks-rotation) bound to its key and to the exact change, and presents it as
`Authorization: Bearer` on `POST` / `DELETE /allowlist`. The CDS re-verifies the token signature,
...
1. A successor replica starts, generates an ephemeral **X25519** keypair, attests itself, and
   calls `POST /handoff` on the active CDS, presenting its attestation-bound token and that ephemeral public key.
2. The active CDS verifies the token's signature and checks the recipient's `launch_digest`
...
| `POST` | `/attest-key` | verify evidence + issue an attestation-bound token bound to the caller's key (the allowlist-write / handoff credential) |
| `POST` | `/handoff` | hand the mesh CA to an attested peer replica |
| `GET` `POST` `DELETE` | `/allowlist` | read / add / remove image digests (writes require a bearer token) |

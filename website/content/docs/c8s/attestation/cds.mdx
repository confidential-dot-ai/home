...
[CA handoff](#ca-handoff). (Certificate issuance does **not** use this token — `/attest`
verifies evidence and returns the signed leaf in one step. Allowlist writes use an
[operator token](/docs/c8s/attestation/allowlist#authorizing-mutations), not an EAR.) Key claims:
...
For each write, the [`c8s allowlist` CLI](/docs/c8s/attestation/allowlist#managing-it-with-the-c8s-cli)
...
| `GET` `POST` `PUT` `DELETE` | `/allowlist` | read / add / replace / remove image digests (writes require an [operator token](/docs/c8s/attestation/allowlist#authorizing-mutations)) |

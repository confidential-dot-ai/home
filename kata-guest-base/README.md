...
| `/etc/c8s/ghcr-auth.json` | generated by `scripts/fetch.sh` | docker auth.json for in-guest guest-pull. Empty (anonymous) in stock builds; a pre-staged file (private-mirror creds) is baked as-is. tmpfiles copies it to `/run/image-security/auth.json` at boot — the `file://` path the kata cmdline names. Part of the measured root. See "Guest-pull registry auth" below. |
...
virtio-net), not from a host bind-mount. The c8s images are public, so
that pull is anonymous and needs no credential. For images that do
require auth (e.g. a **private mirror** of the c8s containers, or private
workload images), the guest reads a docker auth.json via this plumbing:

1. **Build** — `scripts/fetch.sh` writes `/etc/c8s/ghcr-auth.json`. Stock
   builds bake an empty `{"auths":{}}` (anonymous; no secret in the
   image). A hand-placed file — real creds for a private mirror — is
   baked as-is.
...
3. **Cmdline** — the puller appends
   ...
   `kata.guestImage.registryAuth` (its **default** — see
   `internal/helmchart/c8s/values.yaml`). The kata-agent reads that file
   and hands the creds to CDH/image-rs.

**Staging real credentials bakes them into the dm-verity root**, which is
covered by the SNP launch measurement: rotating the credential changes
the rootfs bytes → the launch measurement, so it needs an image rebuild +
re-pinned digest, exactly like the bootstrap allowlist. The secret-free
alternative — CDH fetches the auth.json over `kbs://` after the guest
attests — is described in [`../docs/pitfalls.md`](../docs/pitfalls.md)
("ghcr-auth.json").

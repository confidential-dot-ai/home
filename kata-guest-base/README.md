...
| `/etc/c8s/ghcr-auth.json` | generated by `scripts/fetch.sh` (from `READ_PRIVATE_GHCR_TOKEN`) | docker auth.json for in-guest guest-pull of **private** `ghcr.io/lunal-dev` images. tmpfiles copies it to `/run/image-security/auth.json` at boot — the `file://` path the kata cmdline names. Holds a read-only PAT; part of the measured root. See "Private guest-pull" below. |
...
## Private guest-pull (in-guest GHCR auth)

`shared_fs="none"` + `experimental_force_guest_pull=true` means the
kata-agent pulls each **workload** OCI image *inside* the guest (over
virtio-net), not from a host bind-mount. For a **private**
`ghcr.io/lunal-dev` image that in-guest pull is anonymous by default and
401s. To make it work out of the box we bake a docker auth.json into the
measured rootfs:

1. **Build** — `scripts/fetch.sh` writes `/etc/c8s/ghcr-auth.json` from the
   `READ_PRIVATE_GHCR_TOKEN` env (a classic **read-only** `read:packages`
   PAT). CI passes it from the repo secret of the same name
   (`.github/workflows/kata-guest-base.yml`). No token → an empty
   `{"auths":{}}` is baked (anonymous; no secret in the image). A
   hand-placed file is left untouched when the env var is unset.
2. **Boot** — `/run` is tmpfs, so tmpfiles
   (`extra/etc/tmpfiles.d/c8s.conf`) copies the baked file to
   `/run/image-security/auth.json` at `systemd-tmpfiles-setup`
   (before the agent ever runs a `CreateContainer`).
3. **Cmdline** — the puller appends
   `agent.image_registry_auth=file:///run/image-security/auth.json` from
   `kata.guestImage.registryAuth` (now its **default** — see
   `internal/helmchart/c8s/values.yaml`). The kata-agent reads that file
   and hands the creds to CDH/image-rs.

**This deliberately bakes a credential into the dm-verity root**, which is
covered by the SNP launch measurement. It's acceptable only because the
PAT is read-only and the repos/artifacts are private (and grant no more
than anonymous access once public). Consequences, and the secret-free
alternative (`kbs://` after attestation), are in
[`../docs/pitfalls.md`](../docs/pitfalls.md) ("ghcr-auth.json"). Note:
rotating the PAT changes the rootfs bytes → the launch measurement, so it
needs an image rebuild + re-pinned digest, exactly like the bootstrap
allowlist.

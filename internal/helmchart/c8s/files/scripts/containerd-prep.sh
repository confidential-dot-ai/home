...
# Recognise and remove a template a prior run of this prep wrote.
remove_managed_tmpl() {
  reason=$1
  [ -f "$tmpl" ] || return 0
...
case "$imports_count" in
  ...
  1)
    ...
    ;;
  *)
    # Duplicate `imports` keys in the rendered config — containerd's TOML
    # parser rejects this. Caused by an earlier version of this prep
    # creating a template that prepended `imports` on top of a base

...
mkdir -p "/host{{ $root.Values.hostPaths.cacheDir }}"
mkdir -p "/host{{ $root.Values.hostPaths.runtimeDir }}"
...
[plugins."io.containerd.nri.v1.nri"]
  disable = false
  plugin_path = "{{ $root.Values.hostPaths.pluginDir }}"
  plugin_config_path = "{{ $root.Values.hostPaths.configDir }}"
  plugin_registration_timeout = "10s"
  plugin_request_timeout = "2s"
  socket_path = "/var/run/nri/nri.sock"
[plugins."io.containerd.nri.v1.nri.default_validator"]
  enable = true
  required_plugins = ["{{ $root.Values.pluginName }}"]
EOF
}
...
{{- if $cds }}
  push:
    persist_path: {{ printf "%s/pushed.json" $root.Values.hostPaths.cacheDir | quote }}
{{- else }}
  pull:
    url: {{ required "cds.url is required" $root.Values.cds.url | quote }}
    interval: {{ $root.Values.refresh.interval | quote }}
    timeout: "30s"
{{- end }}
  always_allow:
    {{ required "image.digest is required (chart self-allow for installer rollouts)" $root.Values.image.digest | quote }}: {{ printf "%s@%s" $root.Values.image.repository $root.Values.image.digest | quote }}
{{- range $digest, $image := $root.Values.bootstrapWhitelist.digests }}
    {{ $digest | quote }}: {{ $image | quote }}
{{- end }}
containerd:
  socket: {{ include "nri-image-policy.containerdSocket" $root | quote }}
  namespace: {{ $root.Values.containerd.namespace | quote }}
policy:
  mode: {{ $root.Values.policy.mode | quote }}
  enforce_existing: {{ $root.Values.policy.enforceExisting }}
  deny_missing_annotation: {{ $root.Values.policy.denyMissingAnnotation }}
  exempt_namespaces:
{{- range $root.Values.policy.exemptNamespaces }}
    - {{ . | quote }}
{{- end }}
  label_rules:
{{- if $root.Values.policy.labelRules }}
{{- toYaml $root.Values.policy.labelRules | nindent 4 }}
{{- else }}
    []
{{- end }}
logging:
  level: {{ $root.Values.logLevel | quote }}
{{- end }}

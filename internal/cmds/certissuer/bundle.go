...
// In CDS mode, the bundle is persisted to the repository directory
...
type bundleManager struct {
	...
	// repoDir is the local path to the CDS repository for write-back.
...
// loadFromRepo loads the CA bundle from the CDS repository directory.
...
// subject to 2x maxTTL trimming, and persists to the CDS repository.
func (bm *bundleManager) rotate(newCACert *x509.Certificate) error {
	bm.mu.Lock()
	defer bm.mu.Unlock()

	retained, dropped := certutil.TrimExpiredCABundle(bm.certs, time.Now().Add(-2*bm.maxTTL))
	...
	// Persist bundle to the CDS repository.
...
// persistLocked writes the bundle to the CDS repository. Caller must hold bm.mu.
func (bm *bundleManager) persistLocked() error {
	...
	bm.logger.Debug("persisted CA bundle to CDS repository",
		"path", bundleFile,
		"count", len(bm.certs),
	)
	return nil

...
// The pvc driver shells out to kubectl; if it isn't installed the operator
// must get a clear, actionable error rather than a raw exec failure.
func TestKubectlRunMissingBinary(t *testing.T) {
	t.Setenv("PATH", t.TempDir()) // no kubectl here
	_, err := kubectlRun(context.Background(), "", "version")
	if err == nil || !strings.Contains(err.Error(), "kubectl is required") {
		t.Fatalf("err = %v, want a clear kubectl-not-found error", err)
	}
}
